...
double dc(double z){
    ...
    //printf("/* great_module.great_function */\n");
...
int main(int argc, char* argv[]){
   ...
   static long double DD_total=0,DR_total=0,RR_total=0;
   char* outputfilename = argv[6];
   char* TotPts = argv[7];
   static long double Distribution[RESs][RESu];
...
//printf("buffer value: %f %f\n",buffer2[3][0],buffer[3][0]);
//TODO
    ...
    if(opt==1)//DD,self
    {
        ...
        for(ii=0;ii<flag1;ii++)//DD
            for(jj=ii+1;jj<flag2;jj++){
                    ...
                    DD_total+=(long double)((buffer[3][ii])*(buffer2[3][jj]));
                    ...
                    if(ibuff>=0 && ibuff<RESs && ibuff2>=0 && ibuff2<RESu){
                        Distribution[ibuff][ibuff2]+=(long double)((buffer[3][ii])*(buffer2[3][jj]));
                
            }}
        printf("DD total:%LF\n",DD_total);
        fpXX=fopen(outputfilename,"w+");
        
        for(ii=0;ii<RESs;ii++){
            for(jj=0;jj<RESu;jj++){
                fprintf(fpXX,"%LF ",Distribution[ii][jj]);
            }
            fprintf(fpXX,"\n");
        }
        fclose(fpXX);
    ...
    if(opt==3)//DR
    {
        ...
        for(ii=0;ii<flag1;ii++)//DR
            for(jj=0;jj<flag2;jj++){
                    DR_total+=(long double)((buffer[3][ii])*(buffer2[3][jj]));
                    buff=Separation2(Cos1[ii],Sin1[ii],r1_2[ii],buffer[2][ii],Cos2[jj],Sin2[jj],r2_2[jj],buffer2[2][jj],buffer[1][ii]-buffer2[1][jj]);
                    buff2=Orientation(buffer[2][ii],buffer2[2][jj],buff);
                    ibuff=floor(buff/8);ibuff2=floor(buff2/0.01);
                    if(ibuff>=0 && ibuff<RESs && ibuff2>=0 && ibuff2<RESu){
