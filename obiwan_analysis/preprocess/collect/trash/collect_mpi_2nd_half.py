...
class MyApp(object):
    ...
    def run(self, tasks=None):
        ...
        import glob
        from astropy.table import vstack
        paths = glob.glob(os.path.join(os.environ['my_ngc_run'],'tractor','*','*'))[5000:]
        final_tab = None
        final_sim = None
        n=0
        paths.sort()
        ...
        final_table.write(os.path.join(os.environ['obiwan_out'],'subset','my_ngc_run_chunk23.fits'), format='fits',overwrite=True)
        final_sim.write(os.path.join(os.environ['obiwan_out'],'subset','sim_my_ngc_run_chunk23.fits'), format='fits',overwrite=True)
        print('done!')
...
class MySlave(Slave):
    ...
    def do_work(self, data):
        ...
        tab,sim = production_run_general_perbrick(task_arg,env_dir='my_ngc_run',rs_type='more_rs0',region='ngc')
