...
class MyApp(object):
    ...
    def run(self, tasks=None):
        ...
        import glob
        from astropy.table import vstack
        paths = glob.glob(os.path.join(os.environ['elg_like_run'],'tractor','*','*'))
        final_tab = None
        final_sim = None
        n=0
        paths.sort()
        ...
        final_table.write(os.path.join(os.environ['obiwan_out'],'subset','elg_like_run_chunk21.fits'), format='fits',overwrite=True)
        final_sim.write(os.path.join(os.environ['obiwan_out'],'subset','sim_elg_like_run_chunk21.fits'), format='fits',overwrite=True)
        print('done!')
...
class MySlave(Slave):
    ...
    def do_work(self, data):
        ...
        tab,sim = production_run_general_perbrick(task_arg,env_dir='elg_like_run',rs_type='rs0',region='sgc')
