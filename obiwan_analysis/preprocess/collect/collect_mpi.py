...
class MyApp(object):
    ...
    def run(self, tasks=None):
        ...
        #import glob
        #from astropy.table import vstack
        #paths = glob.glob(os.path.join(os.environ['production_run_sgc'],'*','*'))
        #final_tab = None
        #final_sim = None
        #n=0
        #for path in paths:
        ...
        final_table.write(os.path.join(os.environ['obiwan_out'],'subset','sgc_run_obiwan.fits'), format='fits',overwrite=True)
        final_sim.write(os.path.join(os.environ['obiwan_out'],'subset','sgc_run_sim.fits'), format='fits',overwrite=True)
        print('done!')
...
class MySlave(Slave):
    ...
    def do_work(self, data):
        ...
        tab,sim = production_run_general_perbrick(task_arg,env_dir='production_run_sgc',rs_type='more_rs0',region='sgc')
