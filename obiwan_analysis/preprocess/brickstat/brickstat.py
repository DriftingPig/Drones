...
def OneBrickClassify(brickname):
    ...
    log_dir = '/global/cscratch1/sd/huikong/obiwan_Aug/repos_for_docker/obiwan_out/%s/logs/%s/%s/log.%s'%(NAME_FOR_RUN,brickname[:3],RS,brickname)
...
def BrickClassify():
    ...
    f2.close()
    bricks = np.loadtxt('./elg_real_brick_lists/%s'%REAL_BRICKS_FN, dtype=np.str)
