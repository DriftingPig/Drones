...
def CorrFunc(Njob=Njob,k0=1, order=0,name_corr = filename):
    ...
    for i in range(0,Sep_interval):
        x[i]=MinAngle+i*AngleInteval
    #import pdb
    #pdb.set_trace()
    np.savetxt(output_fn.replace('3d/',''),np.vstack((x,Final_total,CorrFunc_Err)))
    #plt.errorbar(x, Final_total, yerr=CorrFunc_Err)
    #plt.gca().set_xscale("log", nonposx='clip')
    plt.errorbar(x, Final_total,CorrFunc_Err,marker = 'o')
    ...
    plt.gca().set_xlim((MinAngle*0.9, MaxAngle*1.1))
    #plt.gca().set_ylim((-0.1, 0.25))
    plt.savefig(output_fn.replace('.out','.png').replace('3d/',''))
    plt.show()
    return True
