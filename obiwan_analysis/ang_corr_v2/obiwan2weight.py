...
class pixel_conversion(object):
    ...
    def weight_adder(self,fn_dat, fn_sim = None):
        if self.pixel_id is None:
            self.radec2pixdens()
...
def main_add_weight(name,res, fn_sim = None):
    '''
    ...
    eval('survey_name.'+name+'()')
    target = pixel_conversion(survey_name.raw_topdir+survey_name.obiwan_name,survey_name.raw_topdir+survey_name.uniform_name,res)
    target.weight_adder(survey_name.raw_topdir+survey_name.data_name, fn_sim)

def main(mode,name = 'elg_ngc_run',Type=None): 
      if mode == 1:#make plot
          main_plot(name,128,Type,shw_plt=False,plt_name='obiwan_density_chunk23.png')
      if mode == 2: #make weighted column, specify which sim file you want, because we want other weight columns for 3d corr
          fn_sim = '/global/cscratch1/sd/huikong/obiwan_Aug/repos_for_docker/obiwan_out/subset/eBOSS_ELG_clustering_eboss23_v4.ran.fits'
          main_add_weight(name,128, fn_sim)
main(1,name = 'elg_ngc_run',Type='obiwan')
